...
# Słownik do przechowywania zmiennych w czasie rzeczywistym
variables = {}
...
def p_factor(p):
    ...
    if len(p) == 2:
        ...
        elif isinstance(p[1], (int, float, str)): p[0] = p[1]
        else: p[0] = ('VAR', p[1])
...
# --- BARDZO PROSTY EWALUATOR (Wykonawca kodu) ---
def evaluate_expression(expr):
    ...
    if op == 'VAR':
        ...
        operator, left, right = expr[1], expr[2], expr[3]
        ...
        # Tutaj mozna dodac wiecej operatorow
    return 0
...
def execute_ast(node):
    ...
    op = node[0]
    if op == 'PROGRAM':
        execute_ast(node[2]) # Uruchamia main_block (funkcje pomijamy w tym uproszczonym modelu)
    elif op == 'BLOCK':
        for stmt in node[1]:
            if stmt: execute_ast(stmt)
    elif op == 'WRITE':
        val = evaluate_expression(node[1])
        print(val)
    elif op == 'READ':
        val = input(f"{node[1]} = ")
        # Proba rzutowania na int dla wygody
        try: variables[node[1]] = int(val)
        except ValueError: variables[node[1]] = val
    elif op == 'ASSIGN':
        variables[node[1]] = evaluate_expression(node[2])
    elif op == 'IF':
...
def run_interpreter(code):
    ...
    # print(ast) # Odkomentuj to, żeby zobaczyć jak parser widzi Twój kod
    if ast:
        execute_ast(ast)
